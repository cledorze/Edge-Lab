#!/bin/bash
# Add a new SUSE Edge 3.3 node (EIB 3.3) and onboard it to Rancher.
# Optionally prepare SUC labels for OS/K8s upgrades.
#
# Usage:
#   export EIB_33_DIR=/path/to/EIB-3.3
#   ./13-add-edge33-node.sh
...
check_prerequisites() {
    ...
    if [ -z "$EIB_33_DIR" ] || [ ! -d "$EIB_33_DIR" ]; then
        log_error "EIB_33_DIR not set or invalid. Set it to the Edge 3.3 EIB directory."
        exit 1
    fi
...
reg_name="${site_id}-edge33-${role_choice}-reg-${scale_id}"
...
config_out="${ELEMENTAL_DIR}/elemental_config-${site_id}-edge33-${role_choice}-${scale_id}.yaml"
download_config "$reg_url" "$config_out"
add_install_and_reset "$config_out"

iso_out="${OUTPUT_DIR}/vm-rancher-edge33-${site_id}-${role_choice}-${scale_id}.iso"
build_iso "$config_out" "$iso_out"

vm_name="${site_id}-edge33-${role_choice}-${scale_id}"
log_info "Creating VM: $vm_name"
create_vm "$vm_name" "$iso_out"

log_info "OK: Edge 3.3 node VM created and onboarding started."
log_info "SUC labels added: edge-version=3.3, suc-group=edge33"
log_info "Next step: apply SUC plans targeting label suc-group=edge33 (OS + K8s)."
